...
import { DeleteOrdersForDispatchComponent } from './orders-for-dispatch-dialogs/delete-orders-for-dispatch/delete-orders-for-dispatch.component';

@Component({
  selector: 'app-orders-for-dispatch',
  templateUrl: './orders-for-dispatch.component.html',
  styleUrls: ['./orders-for-dispatch.component.css']
})
export class OrdersForDispatchComponent implements OnInit {
    ...
    .subscribe(order => {
      ...
      this.orders.forEach(eachOrder => {
        ...
        console.log(eachOrder)
          ...
          for (const key in eachOrder.deletedOrders) {
            if (eachOrder.deletedOrders.hasOwnProperty(key)) {
              const element = eachOrder.deletedOrders[key];
              eachDeletedSampleArr.push(element);
            }
          }
        eachOrder.fragrances = eachOrderArr;
        eachOrder.deletedOrders = eachDeletedSampleArr;  
...
checkPaymentStatus (order){
  ...
  attendTo(order) {
    this.mobileAppDB.deliverOrder(order);
    order['attendedTo'] = "delivered";
    console.log(order);
  }

  
  returnAllOrder(order) {
    this.mobileAppDB.returnOrder(order);
    order['attendedTo'] = "returned";
    console.log(order);
  }

  showDeliverBtn(order) {
   
    if (order.attendedTo == 'delivered') {
      this.textBtn = 'Delivered';
    } else {
      this.textBtn = 'Deliver Order';
    }
    return this.textBtn;
  }
  ...
  showReturnBtn(order) {
    ...
    if (order.attendedTo == 'returned') {
      this.textBtn = 'Order Returned';
    } else {
      this.textBtn = 'Return All Order';
    }
    return this.textBtn;
  }
...
openDeleteOrderDialog(frag, order): void {
  const dialogRef = this.dialog.open(DeleteOrdersForDispatchComponent, {
    width: 'auto',
    height: 'auto',
    data: {
